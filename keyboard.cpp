...
bool key_trigger[256];		//トリガーキーの状態を保存するbool型配列(キーの最大数は 0x00 ～ 0xff → 256)
...
//キーを1回押して離した際にtrueとなるトリガーキー処理関数
bool Keyboard_TriggerKeyDown(Keyboard_Keys key)
{
	//押されたときの処理
	if (Keyboard_IsKeyDown(key) == true)
	{
		key_trigger[key] = true;
	}

	//離したときの処理
	if (Keyboard_IsKeyUp(key) == true && key_trigger[key] == true)
	{
		key_trigger[key] = false;
		return	true;
	}
